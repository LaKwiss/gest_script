...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    ...
    return openDatabase(
      path,
      version: 5, // Augmenter la version de la BDD
      onCreate: _createDB,
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 4) {
      await db.execute(
        'ALTER TABLE scripts ADD COLUMN is_scheduled '
    ...
    if (oldVersion < 5) {
      await db.execute(
        'ALTER TABLE scripts ADD COLUMN scheduled_params_json TEXT',
      );
    }
  }
